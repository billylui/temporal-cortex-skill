...
All sub-skills share the [Temporal Cortex MCP server](https://github.com/temporal-cortex/mcp) (`@temporal-cortex/cortex-mcp@0.5.7`), a compiled Rust binary distributed as an npm package.
...
**Docker containment:** For isolated execution without Node.js on the host, use the Docker transport:
...
Layer 1 tools work immediately with zero configuration. Calendar tools require a one-time OAuth setup — run the [setup script](https://github.com/temporal-cortex/skills/blob/main/scripts/setup.sh) or `npx @temporal-cortex/cortex-mcp@0.5.7 auth google`.
